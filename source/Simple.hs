...
convertTerm (CoreTerm _ ProofCopyFunction) = simpleFailTerm
convertTerm (CoreTerm _ ProofCopyNumber) = simpleFailTerm
convertTerm (CoreTerm _ (ProofCopyPair _ _)) = simpleFailTerm
convertTerm (CoreTerm _ ProofCopyReference) = simpleFailTerm
...
convertTermAnnotate e (CoreTypeScheme _ (MonoType σ)) = do
  ...
  pure (e', σ')
convertTermAnnotate e (CoreTypeScheme _ (Forall (Bound (Pattern _ x κ) σ))) = withReaderT (Map.insert x κ) $ convertTermAnnotate e σ
