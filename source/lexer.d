...
struct Keyword{
	enum list=[
		...
		//"value_template",
		"while",
		//"yield"//coroutine, acts like return but on next call will continue from yield point
	];
	size_t index;
	string toString(){
		return list[index];
		...
		if(file.front=='\\'){
			...
			while(true){
				if(file.front=='\\'){
					...
					if(file.front=='a'){old~='\a';}
					else if(file.front=='b'){old~='\b';}
					else if(file.front=='f'){old~='\f';}
					else if(file.front=='n'){old~='\n';}
					else if(file.front=='r'){old~='\r';}
					else if(file.front=='t'){old~='\t';}
					else if(file.front=='v'){old~='\v';}
					else if(file.front=='\\'){old~='\\';}
					else if(file.front=='\''){old~='\'';}
					else if(file.front=='"'){old~='"';}
					else if(file.front=='`'){old~='`';}
					else if(file.front=='0'){old~='\0';}
					else if(file.front=='-'){
						file.popFront;
						if(file.empty){
...
struct Token{
	Position pos;
	Algebraic!(Identifier,IntLiteral,Keyword,Operator,CharLiteral,StringLiteral,Eof) value;alias value this;
	...
	@property auto ref expectT(T...)(){
		...
		error("Expected "~T.stringof,pos);return typeof(return).init;
	}
...
void cleanComment(ref string file){
	...
	if(file.front=='#'){
		file.popFront; if(file.empty) return;
		if(file.front=='$'){//multiline comment
			...
			do{
				...
				if(file.front=='#'){
					auto cpy=file;
					cpy.popFront; if(cpy.empty) return;
					if(cpy.front=='$'){
						cpy.popFront;
						file=cpy;
						count++;
...
@trusted unittest{
	...
	assert(lexer.front==IntLiteral(BigInt("1234")));lexer.popFront;
	assert(lexer.front==oper!"(");lexer.popFront;
	assert(lexer.front==oper!"+");lexer.popFront;
	assert(lexer.front==Identifier("hi"));lexer.popFront;
	assert(lexer.front==CharLiteral("char literal"));lexer.popFront;
	assert(lexer.front==StringLiteral("string\tliteral"));lexer.popFront;
	assert(lexer.front==Eof());assert(lexer.empty);
}
