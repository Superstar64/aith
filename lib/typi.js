...
//used for the offset of a struct
libtypi.offsetPointer=function(source,index){
	var ret=function(arg){
		if(arg===undefined){
			return libtypi.array.get(source,index);
		}else{
			return libtypi.array.set(source,index,libtypi.copy(arg));
		}
	}
	ret.pointer=true;
	ret.offset=index;
...
libtypi.copy=function(value){
	...
	if(typeof value=="object"){
		if(value.struct){
			return libtypi.array.realloc(value);
		}
...
libtypi.equal=function(v1,v2){
	...
	if(type=="number"||type=="boolean"||type=="function"||type=="string"){
		if(type=="function"&&v1.offset){
			return (v1()==v2()) && (v1.offset==v2.offset);
		}
		return v1==v2;
	}
	if(type=="object"){
