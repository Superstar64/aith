...
\begin{gather*}
\frac
{\Phi, \alpha : \kappa + \{ \overline c \} \succeq \{ \overline \pi \} | \Gamma \vdash e : \sigma \quad \overline {\Phi \vdash c : \kappa \to \diamond} \quad
\overline { \Phi \vdash \pi : \kappa }}
...
\begin{gather*}
\frac
{\Phi | \Gamma \vdash e : (\sigma *^\pi)^\pi \quad \Phi \vdash \text{Copy} (\sigma) \quad \Phi \vdash \sigma : +^{\text{Real}(\rho)}}
...
{\Phi \vdash \sigma *^\pi : +^{\text{Real}(\text{Pointer})}}
\end{gather*}
...
{\Phi \vdash (\sigma, \tau) : +^{\text{Real} (\text{Struct} \{ \kappa,\kappa'\} )}} \\
\end{gather*}
