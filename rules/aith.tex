...
\begin{gather*}
\frac
{\Phi | \Gamma \vdash e : (\sigma *^{\pi'})^\pi \quad \Phi \vdash \text{Copy} (\sigma) \quad \Phi \vdash \sigma : +^{\text{Real}(\rho)} \quad \pi \succeq \pi'}
{\Phi | \Gamma \vdash *^{\pi'} \, e : \sigma^\pi} \\
\end{gather*}
