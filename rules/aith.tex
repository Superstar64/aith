...
\begin{gather*}
\frac
{\splitfrac{\Phi | \Gamma \vdash e : (\sigma \text{* @ } \, {\pi'})^\pi \quad \Phi \vdash \text{Copy} (\sigma) \quad \pi \succeq \pi'}
{\Phi \vdash \pi : \text{Region} \quad \Phi \vdash \pi' : \text{Region} \quad \Phi \vdash \sigma : +^{\rho}}}
{\Phi | \Gamma \vdash * (e :: \sigma \text{* @}  \, {\pi'})  : \sigma^\pi} \\
\\
\frac
{\splitfrac{\Phi | \Gamma \vdash e : (\sigma \text{* @ } \, {\pi'})^\pi \quad \Phi | \Gamma \vdash e' : \sigma^\pi  \quad \Phi \vdash \text{Copy} (\sigma) \quad \pi \succeq \pi'}
{\Phi \vdash \pi : \text{Region} \quad \Phi \vdash \pi' : \text{Region} \quad \Phi \vdash \sigma : +^{\rho}}}
{\Phi | \Gamma \vdash *(e :: \sigma \text{* @ } \,{\pi'}) = e' : ()^\pi }
\end{gather*}
...
\begin{align*}
e \Coloneqq & \\
& \text{\&*} e \tag{Reinterpret} \\
& e \text{\&+} e' \tag{Pointer Increment} \\
& e \text{\&-} e' \tag{Pointer Difference} \\
\sigma \Coloneqq & \\
& \sigma []
\end{align*}
...
\begin{gather*}
\frac
{\Phi | \Gamma \vdash e : (\sigma \text{[] @ } \pi')^\pi}
{\Phi | \Gamma \vdash e : (\sigma \text{* @ } \pi')^\pi} \\
\\
\frac
{\Phi | \Gamma \vdash e : (\sigma \text{[] @ } \pi')^\pi \quad \Phi |
 \Gamma' \vdash e' : (\#\text{unsigned} \, \text{native})^\pi}
{\Phi | \Gamma, \Gamma' \vdash e \text{\&+} e' : (\sigma \text{[] @ } \pi')^\pi} \\
\\
\frac
{\Phi | \Gamma \vdash e : (\sigma \text{[] @ } \pi')^\pi \quad \Phi |
 \Gamma' \vdash e' : (\sigma \text{[] @ } \pi')^\pi}
{\Phi | \Gamma, \Gamma' \vdash e \text{\&-} e' : (\#\text{unsigned} \, \text{native})^\pi}
\end{gather*}
...
{\Gamma \vdash e \text{\&+}^\sigma e' : \text{void}*} \\
\\
\frac
{\Gamma \vdash e : \text{void}* \quad \Gamma \vdash e' : \text{void}*}
{\Gamma \vdash e \text{\&-}^\sigma e' : \text{word} \, \text{size}}
\end{gather*}
