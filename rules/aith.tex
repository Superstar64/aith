...
\begin{align*}
e \Coloneqq & \\
& *^\pi e \tag{Read Pointer} \\
& *^\pi e = (e' : \sigma) \tag{Write Pointer} \\
& \dots \\
\sigma, \tau, \pi \Coloneqq & \\
...
\\
\frac
{\Phi | \Gamma \vdash e : (\sigma *^{\pi'})^\pi \quad \Phi | \Gamma \vdash e' : \sigma^\pi  \quad \Phi \vdash \text{Copy} (\sigma) \quad \pi \succeq \pi \quad \Phi \vdash \sigma : +^\rho}
{\Phi | \Gamma \vdash *^{\pi'} e = (e' : \sigma) : ()^\pi }
\end{gather*}
...
{\Phi | \Gamma, \Gamma' \text{if} (e) \{ e' \} \text{else} \{ e'' \} : \sigma^\pi }
\end{gather*}
